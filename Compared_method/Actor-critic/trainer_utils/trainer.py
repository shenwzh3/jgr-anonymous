...
class Trainer:
    ...
    def compute_loss_reranker(self, generator_model, reranker_model, inputs, return_outputs=False):
        ...
        _, candidate_texts, candidate_scores = self.compute_metrics.get_candidates(target_text,  generated_seqs, 
                                1, 1, self.args.candidate_pick_strategy) # (B* (C-1)), list with length of B*C
        
        # self.reward_tracker['scores'].append(torch.var(candidate_scores, dim=1).mean().item())
        
        # process input for input
        reranker_input_ids, reranker_attention_mask = self._get_reranker_input_ids(source_text, candidate_texts) # both (B, 1, L)

        reranker_output = reranker_model(input_ids=reranker_input_ids, attention_mask = reranker_attention_mask, labels = candidate_scores)
    ...
    def compute_loss_generator(self, generator_model, reranker_model, inputs, return_outputs=False):
        ...
        candidate_indices, candidate_texts, candidate_scores = self.compute_metrics.get_candidates(target_text, generated_seqs, 
                                1, 1,  self.args.candidate_pick_strategy) # (B* C), list with length of B*C
        ...
        with torch.no_grad():
            ...
            reranker_rewards = reranker_logits # (B, C)
    ...
    def evaluation_loop(
        ...
        for step, inputs in enumerate(dataloader):
            ...
            # Prediction step
            generator_loss,  generator_logits, reranker_logits, labels = self.prediction_step(generator_model, reranker_model, inputs, prediction_loss_only, ignore_keys=ignore_keys)
            ...
            if self.args.eval_accumulation_steps is not None and (step + 1) % self.args.eval_accumulation_steps == 0:
                ...
                generator_losses_host,  generator_preds_host, reranker_preds_host, labels_host = None, None, None
    ...
    def prediction_step(
        ...
        with torch.no_grad():
            ...
            if is_sagemaker_mp_enabled():
                ...
                _, reranker_outputs = self.compute_loss_reranker_eval(reranker_model, reranker_inputs, return_outputs=True)
        ...
        if prediction_loss_only:
            return (generator_loss, None, None, None)
        ...
        return (generator_loss,  generator_preds, reranker_preds, targets)
